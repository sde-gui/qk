...
foo_canvas_widget_destroy (GtkObject *object)
{
	...
	if (GTK_OBJECT_CLASS (foo_canvas_widget_parent_class)->destroy)
		GTK_OBJECT_CLASS (foo_canvas_widget_parent_class)->destroy (object);
}
...
foo_canvas_widget_update (FooCanvasItem *item, double i2w_dx, double i2w_dy, int flags)
{
	...
	if (FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->update)
		FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->update (item, i2w_dx, i2w_dy, flags);
...
foo_canvas_widget_map (FooCanvasItem *item)
{
	...
	if (FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->map)
		FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->map (item);
...
{
	FooCanvasWidget *witem = FOO_CANVAS_WIDGET (item);
	if (FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->unmap)
		FOO_CANVAS_ITEM_CLASS (foo_canvas_widget_parent_class)->unmap (item);
