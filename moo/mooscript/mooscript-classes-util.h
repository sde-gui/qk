...
Type get_arg_func(const Variant &var, const char *)             \
{                                                               \
    if (var.vt() != Vt)                                         \
        Error::raise("invalid argument value");                 \
    return var.value<Vt>();                                     \
}                                                               \
Type get_arg_func##_opt(const Variant &var, const char *)       \
{                                                               \
    if (var.vt() == VtVoid)                                     \
        return Type();                                          \
    else if (var.vt() != Vt)                                    \
...
Variant wrap_func(const Type &val)                              \
{                                                               \
    return Variant(val);                                        \
}
...
gint64 get_arg_int(const Variant &var, const char *)
{
    switch (var.vt())
    {
        case VtInt:
...
Variant wrap_int(gint64 value)
{
    return Variant(value);
}

gint64 get_arg_index(const Variant &var, const char *)
{
    switch (var.vt())
    {
        case VtInt:
...
Variant wrap_index(gint64 val)
{
    return Variant(Index(val));
}

Variant get_arg_variant(const Variant &var, const char *)
{
    return var;
}

Variant get_arg_variant_opt(const Variant &var, const char *)
{
    return var;
}

template<typename T>
static T *get_object_arg_impl(const Variant &val, bool null_ok, const char *)
{
...
template<typename T>
T &get_object_arg(const Variant &var, const char *argname)
{
    return *get_object_arg_impl<T>(var, false, argname);
}

template<typename T>
T *get_object_arg_opt(const Variant &var, const char *argname)
{
    return get_object_arg_impl<T>(var, true, argname);
}

Variant wrap_object(Object *obj)
{
