...
unset_data (lua_State *L)
{
    ...
    {
        data = (MooEditLuaData*) lua_touserdata (L, -1);
        g_free (data);
        lua_pop (L, 1);
...
void
_moo_edit_lua1_add_api (lua_State *L)
{
...
void
_moo_edit_lua1_set_doc (lua_State   *L,
                        GtkTextView *doc)
{
    MooEditLuaData *data;
...
void
_moo_edit_lua1_cleanup (lua_State *L)
{
    g_return_if_fail (L != NULL);
    unset_data (L);
}
...
cfunc_open (lua_State *L)
{
    ...
    editor = MOO_IS_EDIT (data->doc) ? moo_edit_get_editor (MOO_EDIT (data->doc)) : moo_editor_instance ();
