 ...
 * Changed by Muntyan
 *
 * 03/28/2005: Added copying tags
 * 06/04/2005: Increased KEYWORD_LIMIT to 1000
 *
 *****************************************************************************/
...
static GtkSourceTag	*gtk_syntax_tag_copy 		(GtkSyntaxTag       *text_tag);
...
static GtkSourceTag	*gtk_pattern_tag_copy 		(GtkPatternTag      *text_tag);
...
GtkTextTag *
gtk_source_tag_copy (GtkSourceTag *tag)
{
	GType type;
	GtkSourceTag *new_tag;
	GtkSourceTagStyle *style;

	g_return_val_if_fail (GTK_IS_SOURCE_TAG (tag), NULL);

	type = G_TYPE_FROM_INSTANCE (tag);
	if (type == GTK_TYPE_SYNTAX_TAG)
		new_tag = gtk_syntax_tag_copy (GTK_SYNTAX_TAG (tag));
	else if (type == GTK_TYPE_PATTERN_TAG)
		new_tag = gtk_pattern_tag_copy (GTK_PATTERN_TAG (tag));
	else
	{
		g_critical ("Can't copy source tag of unknown type %s",
			     g_type_name (type));
		return NULL;
	}
	...
	if (style)
	{
		gtk_source_tag_set_style (new_tag, style);
		gtk_source_tag_style_free (style);
	}

	return GTK_TEXT_TAG (new_tag);
}
		    ...
		    const gchar *pattern_end)
{
	...
	if (tag->reg_end == NULL) {
		...
		g_free (tag->start);
		gtk_source_regex_unref (tag->reg_start);
		g_object_unref (tag);
		return NULL;
	}
...
static GtkSourceTag *
gtk_syntax_tag_copy (GtkSyntaxTag *tag)
{
	GtkSyntaxTag *new_tag;
	gchar *name;

	g_object_get (G_OBJECT (tag), "name", &name, NULL);
	new_tag = GTK_SYNTAX_TAG (g_object_new (GTK_TYPE_SYNTAX_TAG,
						"id", GTK_SOURCE_TAG(tag)->id,
						"name", name,
						NULL));
	g_free (name);

	new_tag->start = g_strdup (tag->start);
	new_tag->reg_start = gtk_source_regex_ref (tag->reg_start);
	new_tag->reg_end = gtk_source_regex_ref (tag->reg_end);

	return GTK_SOURCE_TAG (new_tag);
}
...
gtk_syntax_tag_finalize (GObject *object)
{
	...
	gtk_source_regex_unref (tag->reg_start);
	gtk_source_regex_unref (tag->reg_end);
...
static GtkSourceTag *
gtk_pattern_tag_copy (GtkPatternTag *tag)
{
	GtkPatternTag *new_tag;
	gchar *name;

	g_object_get (G_OBJECT (tag), "name", &name, NULL);
	new_tag = GTK_PATTERN_TAG (g_object_new (GTK_TYPE_PATTERN_TAG,
						 "id", GTK_SOURCE_TAG(tag)->id,
						 "name", name,
						 NULL));
	g_free (name);

	new_tag->reg_pattern = gtk_source_regex_ref (tag->reg_pattern);
...
gtk_pattern_tag_finalize (GObject *object)
{
	...
	gtk_source_regex_unref (tag->reg_pattern);
