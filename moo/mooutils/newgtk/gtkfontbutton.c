 ...
 */
/*
 * Taken from gtk-2.6.1 for gtk < 2.4. Added gtk version check to avoid compiling
 * it with recent gtk versions, and added noop P_() and _().
 */
...
{
  font_button->priv = g_new0 (GtkFontButtonPrivate, 1);
...
gtk_font_button_finalize (GObject *object)
{
  ...
  if (font_button->priv != NULL)
  {
    ...
    g_free (font_button->priv);
    font_button->priv = NULL;
  }
