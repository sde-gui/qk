...
UGLY_DEPS =					\
	$(UGLY)					\
	$(top_srcdir)/ugly/ugly-pre.mk	\
	$(top_srcdir)/ugly/ugly-post.mk	\
	$(top_srcdir)/ugly/ugly-sub.mk	\
	$(top_srcdir)/ugly/bdist.mk
...
ugly-subdirs-stamp: Makefile $(top_srcdir)/ugly/ugly-subdir-Makefile
	@if test -n "$(UGLY_SUBDIRS)"; then \
	  for d in $(UGLY_SUBDIRS); do \
	    mkdir -p $$d || exit 1; \
	    cp $(top_srcdir)/ugly/ugly-subdir-Makefile $$d/Makefile || exit 1; \
	  done; \
	fi
